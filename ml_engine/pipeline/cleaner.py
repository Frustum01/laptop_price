...
def clean_and_sample_dataset(dataset_dir):
    ...
    quality_score = calculate_data_quality_score(df, missing_ratio, duplicate_ratio, outlier_ratio)
    ...
    if len(train_df) > 500000:
        logger.info(f"[{metadata['dataset_id']}] Dataset > 500k rows. Applying 100k sampling for training.")
    ...
    logger.info(f"[{metadata['dataset_id']}] Cleaning complete. Final rows: {len(df)}, Train Rows: {len(train_df)}")
    return {
        "status": "success",
