...
class SemanticMetricEngine:
    ...
    def build_definitions(self):
        ...
        if sales_col and sales_col in self.df.columns:
            ...
            metrics["monthly_sales_growth"] = {"type": "time_series_growth", "column": sales_col}
        ...
        if ("total_revenue" in metrics and "total_cost" not in metrics) and "total_profit" in metrics:
    ...
    def _evaluate_base_metric(self, metric_name, config):
        ...
        if "column" in config and "aggregation" in config:
            ...
            if col in self.df.columns:
                if agg == "sum": return float(self.df[col].sum())
                if agg == "mean": return float(self.df[col].mean())
                if agg == "max": return float(self.df[col].max())
                if agg == "min": return float(self.df[col].min())
            ...
            if date_col and col and date_col in self.df.columns:
                try:
                    ...
                    monthly = ts.set_index(date_col).resample('M')[col].sum()
                    if len(monthly) >= 2:
                        last = monthly.iloc[-1]
                        prev = monthly.iloc[-2]
                        if prev != 0:
    ...
    def resolve_metric(self, metric_name, visited=None):
        ...
        if "formula" in config:
            ...
            try:
                # Evaluate the raw math expression safely
                val = eval(formula, {"__builtins__": None}, {})
                self.computed_values[metric_name] = round(float(val), 4)
                return self.computed_values[metric_name]
            except Exception as e:
                logger.error(f"Formula evaluation failed for {metric_name} ({config['formula']}): {e}")
                return None
        else:
    ...
    def execute(self):
        ...
        logger.info(f"Metric Engine resolved {len(self.computed_values)} semantic metrics.")
        return True
