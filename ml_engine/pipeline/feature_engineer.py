...
def extract_datetime_features(df, schema):
    ...
    if date_col and date_col in df.columns:
        try:
            ...
            logger.warning(f"Failed to extract datetime features for {date_col}: {str(e)}")
    return df
...
def encode_categorical_features(df, dataset_dir):
    ...
    for col in categorical_cols:
        # If cardinality is extremely high, we might drop it, but cleaner should have handled that.
        # We will label encode to be safe for tree models.
        le = LabelEncoder()
    ...
    if encoders:
        ...
        logger.info(f"Saved label encoders for {len(encoders)} columns to {encoder_path}")
...
def engineer_features(dataset_dir, use_scaler=False):
    ...
    scaled_cols = []
    if use_scaler:
        df, scaled_cols = scale_numerical_features(df, dataset_dir, exclude_cols=encoded_cols)
        logger.info(f"StandardScaler applied to {len(scaled_cols)} columns (linear model mode).")
    else:
    ...
    logger.info(f"Feature engineering complete. Derived: {len(derived_cols)} | Encoded: {len(encoded_cols)} | Scaled: {len(scaled_cols)}")
