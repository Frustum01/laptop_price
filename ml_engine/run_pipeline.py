...
def run_pipeline(file_path, user_id="default_user", dataset_id_input=None):
    ...
    try:
        # ── 2. Schema & Profiling ───────────────────────────────────────────
        schema, _ = _timed_stage("schema_manager", process_schema, val_res["raw_path"], dataset_dir)
        if not schema:
        ...
        train_fn = lambda: train_evaluate_models(dataset_dir, pipeline_mode=pipeline_mode)
        ...
        # ── 8. Metric Engine ────────────────────────────────────────────────
        metric_res, _ = _timed_stage("metric_engine", generate_metric_definitions, dataset_dir)
        if metric_res:
            result_payload["artifacts_generated"].extend(["metrics.json", "metrics_definition.json"])
        ...
        missing = [a for a in EXPECTED_ARTIFACTS if not os.path.exists(os.path.join(dataset_dir, a))]
        if missing:
            artifact_status = "partial_failure"
            result_payload["status"] = "failed"
            result_payload["error"]  = f"artifact_generation_failed: Missing {', '.join(missing)}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_path", type=str, required=True, help="Path to the uploaded dataset file")
    parser.add_argument("--user_id",       type=str, default="default_user", help="Organizational User ID")
    parser.add_argument("--dataset_id",    type=str, required=False,          help="Optional dataset ID from Node backend")
